...
<?php
    ...
    # Flags de erros de banimento
    if (isset($_SESSION["dados_incompletos-banir"])) {
        echo $aviso . "Por favor, preencha todos os campos.</p>";
        unset($_SESSION["dados_incompletos-banir"]);
    }
    if (isset($_SESSION["senha-errada-banir"])) {
        echo $aviso . "A senha digitada não conhecide com a senha de sua conta como administrador.</p>";
        unset($_SESSION["senha-errada-banir"]);
    }
    if (isset($_SESSION["ErroBancoBan"])) {
        echo $aviso . "Ocorreu um erro ao se comunicar com o banco para concluir o banimento.</p>";
        unset($_SESSION["ErroBancoBan"]);
    }
    # Fim das flags de erro de promoção
    ...
    if (isset($_SESSION['SucessoBan'])) {
        echo $aviso2 . "A conta foi banida com sucesso.</p>";
        unset($_SESSION['SucessoBan']);
    }
    if (isset($_SESSION['SucessoDesBan'])) {
        echo $aviso2 . "A conta foi reativada com sucesso.</p>";
        unset($_SESSION['SucessoDesBan']);
    }
    # Fim das flags de erro alteração de dados pessoais do cliente
?>